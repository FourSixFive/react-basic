...
class KakaoMap extends Component {
    ...
    getData() {
		...
		var url = 'https://express-server4.run.goorm.io/openapi/getdata?keyword=' + this.state.keyword + '&pageNo=' +
            this.state.pageNo;;
        fetch(url, { method: 'get' }) //체인방식으로 실행. 장점은 줄 순서대로 각각 실행 결과가 마무리 된 후 다음 줄이 실행 된다.
            ...
            }).catch((err) => console.log('Error : ' + err));//then(content) 끝
    ...
    render() {
        ...
        return (
            <div>
                ...
                {/* {memberList.map((member) => <div style={{border:'1px solid red', backgroundColor:'yellow', textAlign:'center'}} key={member.email}>{member.name}</div>)} */}
                {/*memberList.forEach((member) => <div style={{border:'1px solid red’}}
                key={member.email}>{member.name}</div>) 주석표시 div style 로 세로 데이터 내용을 확인한다. */}
            </div>
        );
    }
}
